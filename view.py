...
def show_contacts(book: dict[int: dict[str, str]], message):
    if book:
        ...
        for index, contact in book.items():
            print(f'{index: >3}. {contact.get("name"): <{size_name+1}} {contact.get("phone"): <{size_phone+1}}'
...
def change_contact(key, result):
    res = list(filter(lambda sub: key in list(sub.keys()), result))
    return res
